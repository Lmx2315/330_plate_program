...
int jtag_scan(const uint8_t *irlens)
{
	...
	jtagtap_init();//инициализируем подсистему JTAG привязав к абстрактным командам реальные реализации TMS,TCK,TDI,TDO

	jtag_proc.jtagtap_reset();//сбрасываем подсистему JTAG
	...
	for(i = 0; i < jtag_dev_count; i++)
	{
		for(j = 0; dev_descr[j].idcode; j++)
			...
			u_out("idcode     :",jtag_devs[i].idcode);
